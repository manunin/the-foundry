...
Базовый `pr_verify` workflow уже есть как отдельный entrypoint: он запускает verifier на существующем task/worktree-контексте и не открывает новый PR. Следующий шаг — полностью связать это с GitHub PR comments/checkout так, чтобы комментарий или failing CI автоматически запускали verification/report цикл.
...
Базовый web-observability слой уже реализован: `task_events` в SQLite, FastAPI projections, SSE `/api/tasks/{id}/events`, React UI со списком задач, stepper'ом и live event stream.

Открытое направление — стоимость и агрегаты: `cost_usd`, `tokens_in`, `tokens_out` уже проходят через agent results/events там, где backend их отдаёт, но UI/аналитика ещё не делают полноценный cost dashboard.
