...
def test_collect_context_forced_mode_references_skill_without_body(
    tmp_path: Path,
) -> None:
    (tmp_path / "openspec").mkdir()
    skill = tmp_path / ".codex" / "skills" / "openspec-plan-change"
    ...
    with patch.object(openspec, "cli_available", return_value=False):
        ctx = openspec.collect_context(tmp_path, timeout_sec=10, forced=True)
    ...
    assert "OpenSpec skill body" not in rendered
...
def test_format_context_renders_openspec_skills_without_cli_commands(
    ...
    ctx = {
        ...
        "forced": True,
        "cli_available": True,
        "skills": openspec._skill_refs(tmp_path),
    }
    ...
    assert ".codex/skills/openspec-plan-change/SKILL.md" in rendered
    assert "Use proposal.md" not in rendered
...
def test_build_implementation_handoff_uses_artifacts_cli_and_skills(
    ...
    assert "OpenSpec artifacts generated by PLAN" in handoff
...
def test_changed_artifact_paths_lists_modified_and_untracked_openspec_files(
    tmp_path: Path,
) -> None:
    status = Result(
        returncode=0,
        stdout=(
            " M openspec/changes/add-api/proposal.md\n"
            "?? openspec/changes/add-api/tasks.md\n"
            " M src/foundry/workflows.py\n"
        ),
        stderr="",
    )

    with patch.object(openspec.shell, "run", return_value=status) as run:
        paths = openspec.changed_artifact_paths(tmp_path)
